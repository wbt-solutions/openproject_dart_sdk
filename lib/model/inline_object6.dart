...
class InlineObject6 {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "InlineObject6[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "InlineObject6[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, List<InlineObject6>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = InlineObject6.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
