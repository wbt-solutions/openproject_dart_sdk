...
class TimeEntriesLinks {
  ...
  static TimeEntriesLinks? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TimeEntriesLinks[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TimeEntriesLinks[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return TimeEntriesLinks(
  ...
  static List<TimeEntriesLinks>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <TimeEntriesLinks>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TimeEntriesLinks.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TimeEntriesLinks> mapFromJson(dynamic json) {
    final map = <String, TimeEntriesLinks>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TimeEntriesLinks.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<TimeEntriesLinks>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TimeEntriesLinks>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TimeEntriesLinks.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
